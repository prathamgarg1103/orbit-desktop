...
Diya is a cursor-native desktop companion for the instant a person gets stuck. In FL Studio, Claude Code, a browser, or another unfamiliar app, press `Ctrl` + `Shift` + `Space`. Diya captures that current display once and appears only as a small, click-through second cursor beside the real pointer. A second press opens the ask panel when it is wanted; the controls do not remain floating while the person works. As the person moves, Diya can identify the accessible control under the pointer, so the interaction stays grounded in the actual interface instead of a detached chat window.

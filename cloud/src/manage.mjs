...
export async function runAdmin({ args = process.argv.slice(2), env = process.env, write = (value) => process.stdout.write(`${value}\n`) } = {}) {
  ...
  try {
    ...
    if (resource === "invite" && action === "list") {
      const result = { invites: await database.listInvites() };
      write(JSON.stringify(result, null, 2));
      return result;
    }
    if (resource === "invite" && action === "revoke") {
      ...
      const result = { id, revoked: await database.revokeInvite(id) };
      write(JSON.stringify(result, null, 2));
      return result;
    }
    if (resource === "waitlist" && action === "list") {
      const status = option(options, "status") || "";
      const waitlist = (await database.listWaitlistEntries(status)).map(({ encryptedEmail, ...entry }) => ({
        ...entry,
    ...
    if (resource === "waitlist" && action === "invite") {
      ...
      const expiresDays = integerOption(options, "expires-days", 30, 1, 365);
      const code = issueInviteCode();
      const issued = await database.createWaitlistInvite({
        waitlistId: id,
    ...
    if (resource === "waitlist" && action === "set-status") {
      ...
      const result = { id, status, updated: await database.updateWaitlistStatus(id, status) };
      write(JSON.stringify(result, null, 2));
      return result;
    }
    if (resource === "feedback" && action === "list") {
      const status = option(options, "status") || "";
      const feedback = (await database.listFeedbackEntries(status)).map(({ encryptedMessage, encryptedEmail, ...entry }) => ({
        ...entry,
    ...
    if (resource === "feedback" && action === "set-status") {
      ...
      const result = { id, status, updated: await database.updateFeedbackStatus(id, status) };
      write(JSON.stringify(result, null, 2));
      return result;
    }
    if (resource === "metrics" && action === "overview") {
      const result = { metrics: await database.operatorMetrics() };
      write(JSON.stringify(result, null, 2));
      return result;
    }
